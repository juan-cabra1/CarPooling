...
const DriverTrackingScreen: React.FC = () => {
  ...
  const loadTripAndBookings = async () => {
    try {
      ...
      const tripBookingsResult = await bookingsService.getMyBookings();
      setBookings(tripBookingsResult.bookings ?? []);

      console.log(`Loaded ${tripBookingsResult.bookings?.length ?? 0} confirmed bookings for trip ${tripId}`);

      // Si ya estamos trackeando este viaje, no reiniciar
      if (isTracking && String(activeTrip?.id) === String(tripId)) {
        console.log('Already tracking trip', tripId, '— skipping startTracking');
        return;
      }
    ...
    <View style={styles.container}>
      ...
      <View style={styles.bottomContainer}>
        <Text style={styles.etaText}>
          {eta
            ? `ETA: ${eta >= 60 ? `${Math.floor(eta / 60)}h ${Math.round(eta % 60)}min` : `${Math.round(eta)} min`}`
            : 'Calculando...'}
        </Text>
        ...
        {/* Botón para confirmar llegada al punto de salida */}
        {isNearOrigin && activeRouteType === 'toOrigin' && (
          <Button onPress={confirmArrivalAtOrigin} style={{ marginBottom: 8, backgroundColor: '#16a34a' }}>
            <Text style={{ color: 'white', fontWeight: 'bold' }}>📍 Llegué al punto de salida</Text>
          </Button>
        )}
