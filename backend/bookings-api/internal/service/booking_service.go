...
type BookingService interface {
	...
	GetTripBookings(ctx context.Context, tripID string, driverID int64) ([]*domain.BookingResponse, error)
...
func (s *bookingService) CreateBooking(ctx context.Context, req domain.CreateBookingRequest) (*domain.BookingResponse, error) {
	...
	// DEV MODE: If dev_mode is true, create as "confirmed" for testing without payments
	// NORMAL MODE: Create in pending state, trips-api will validate and confirm
	...
	if req.DevMode {
		...
		log.Warn().
			Str("trip_id", req.TripID).
			Int64("passenger_id", req.PassengerID).
			Msg("🔧 DEV MODE: Creating booking as CONFIRMED (skipping payment flow)")
	}
	...
	log.Info().
		...
		Bool("dev_mode", req.DevMode).
		Msg("✅ Booking created successfully")

	// Step 4: Publish event to RabbitMQ to notify trips-api
	// DEV MODE: Publish reservation.created so trips-api updates available_seats
	//           The booking is already confirmed, trips-api just needs to decrement seats
	// NORMAL MODE: trips-api will validate and respond with reservation.confirmed or reservation.failed

	// IMPORTANT: Even in dev mode, we MUST publish the event so trips-api updates available_seats
	// Otherwise the trip will always show the same number of available seats
	if err := s.publisher.PublishReservationCreated(
		...
		if req.DevMode {
			log.Info().
				...
				Msg("🔧 DEV MODE: Published reservation.created event - trips-api will update available_seats")
		} else {
			log.Info().
				Str("booking_id", booking.BookingUUID).
				Str("event_type", "reservation.created").
...
// GetTripBookings retrieves all bookings for a specific trip (driver only)
// Authorization: Only the trip driver can view bookings for their trip
func (s *bookingService) GetTripBookings(ctx context.Context, tripID string, driverID int64) ([]*domain.BookingResponse, error) {
	log.Info().
		Str("trip_id", tripID).
		Int64("driver_id", driverID).
		Msg("Getting bookings for trip")

	// Step 1: Verify the user is the driver of this trip
	trip, err := s.tripsClient.GetTrip(ctx, tripID)
	if err != nil {
		log.Error().
			Err(err).
			Str("trip_id", tripID).
			Msg("Failed to get trip details")
		return nil, fmt.Errorf("failed to get trip details: %w", err)
	}

	// Step 2: Authorization check - must be the trip driver
	if trip.DriverID != driverID {
		log.Warn().
			Str("trip_id", tripID).
			Int64("driver_id", driverID).
			Int64("actual_driver_id", trip.DriverID).
			Msg("Unauthorized access to trip bookings")
		return nil, domain.ErrUnauthorized.WithDetails(map[string]interface{}{
			"trip_id":   tripID,
			"driver_id": driverID,
		})
	}

	// Step 3: Get all bookings for this trip
	bookings, err := s.bookingRepo.FindByTripID(tripID)
	if err != nil {
		log.Error().
			Err(err).
			Str("trip_id", tripID).
			Msg("Failed to get bookings for trip")
		return nil, fmt.Errorf("failed to get trip bookings: %w", err)
	}

	// Step 4: Filter to only confirmed bookings (exclude cancelled/failed)
	confirmedBookings := make([]*domain.BookingResponse, 0)
	for _, booking := range bookings {
		if booking.IsConfirmed() {
			confirmedBookings = append(confirmedBookings, domain.ToBookingResponse(&booking))
		}
	}
	...
	log.Info().
		...
		Int("confirmed_bookings", len(confirmedBookings)).
		Msg("Successfully retrieved trip bookings")
