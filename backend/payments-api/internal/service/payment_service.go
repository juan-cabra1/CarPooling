...
type PaymentService interface {
	CreatePreference(userID string, req *domain.CreatePreferenceRequest) (*domain.CreatePreferenceResponse, error)
...
func (s *paymentService) CreatePreference(userID string, req *domain.CreatePreferenceRequest) (*domain.CreatePreferenceResponse, error) {
	...
	// Set back URLs for mobile deep links
	mpReq.BackURLs = &domain.MPBackURLs{
		Success: fmt.Sprintf("carpooling://payment/success?payment_id=%s", payment.PaymentUUID),
		Pending: fmt.Sprintf("carpooling://payment/pending?payment_id=%s", payment.PaymentUUID),
		Failure: fmt.Sprintf("carpooling://payment/failure?payment_id=%s", payment.PaymentUUID),
	}
