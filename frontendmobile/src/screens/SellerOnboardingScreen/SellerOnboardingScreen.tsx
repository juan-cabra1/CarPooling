...
import { View, Text, ScrollView, Linking, Alert, ActivityIndicator, TextInput, TouchableOpacity } from 'react-native'
...
const REDIRECT_URI = 'carpooling://seller/callback'
...
export default function SellerOnboardingScreen() {
  ...
  const setupDeepLinkListener = () => {
    // Listen for deep link callback from MercadoPago OAuth
  ...
  const handleDeepLink = async (event: { url: string }) => {
    const url = event.url
    if (url.startsWith('carpooling://seller/callback')) {
      // Parse the authorization code from the URL
      const urlParams = new URL(url.replace('carpooling://', 'https://'))
  ...
  const handleLinkAccount = async () => {
    try {
      ...
      if (canOpen) {
        ...
        throw new Error('No se pudo abrir el enlace de autorizacion')
      }
    } catch (err) {
      setError(getErrorMessage(err))
      setLinking(false)
