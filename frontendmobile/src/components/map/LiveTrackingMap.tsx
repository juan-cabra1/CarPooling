...
const LiveTrackingMap: React.FC = () => {
  ...
  return (
      ...
      >
        ...
        {/* Ruta desde origen hasta destino (azul) - SIEMPRE visible */}
        {routeCoordinates && routeCoordinates.length > 0 && (
            <Polyline
                coordinates={routeCoordinates.map(p => ({ latitude: p.lat, longitude: p.lng }))}
                strokeColor="#4285F4"
                strokeWidth={5}
                lineDashPattern={activeRouteType === 'toDestination' ? undefined : [1, 10]}
            />
        )}

        {/* Ruta desde ubicación actual hasta origen (naranja) - Solo cuando es la ruta activa */}
