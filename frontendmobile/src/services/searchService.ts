...
const SEARCH_BASE = '/api/v1/search' // Search API uses /api/v1 prefix
...
export async function searchTrips(query: SearchQuery): Promise<SearchResponse<SearchTrip>> {
  ...
  const response = await searchApi.get<ApiResponse<SearchResponse<SearchTrip>>>(url)
...
export async function autocomplete(
  ...
  const response = await searchApi.get<ApiResponse<string[]>>(url)
  return response.data.data!
}
...
export async function getPopularRoutes(limit = 10): Promise<PopularRoute[]> {
  ...
  const response = await searchApi.get<ApiResponse<PopularRoute[]>>(url)
  return response.data.data!
}
...
export async function getTripDetails(id: string): Promise<SearchTrip> {
  ...
  const response = await searchApi.get<ApiResponse<SearchTrip>>(`/api/v1/trips/${id}`)
  return response.data.data!
}
