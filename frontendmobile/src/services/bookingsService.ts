...
const BOOKINGS_BASE = '/api/v1/bookings' // Bookings API uses /api/v1 prefix
...
export async function createBooking(data: CreateBookingRequest): Promise<Booking> {
  const response = await bookingsApi.post<ApiResponse<Booking>>(BOOKINGS_BASE, data)
...
export async function getMyBookings(
  ...
  const response = await bookingsApi.get<ApiResponse<BookingListResponse>>(url)
  return response.data.data!
}
...
export async function getBookingById(id: string): Promise<Booking> {
  const response = await bookingsApi.get<ApiResponse<Booking>>(`${BOOKINGS_BASE}/${id}`)
  return response.data.data!
}

/**
 * Get all confirmed bookings for a specific trip (driver only)
 * Requires authentication - only trip driver can view
 * @param tripId - Trip ID (MongoDB ObjectID)
 * @returns Array of confirmed bookings for the trip
 * @throws Error if trip not found (404) or not authorized (403)
 * @example
 * const bookings = await bookingsService.getBookingsByTrip('507f1f77bcf86cd799439011')
 */
export async function getBookingsByTrip(tripId: string): Promise<Booking[]> {
  const response = await bookingsApi.get<ApiResponse<Booking[]>>(`/api/v1/trips/${tripId}/bookings`)
  return response.data.data!
}
...
export async function cancelBooking(id: string, reason?: string): Promise<void> {
  ...
  await bookingsApi.patch(`${BOOKINGS_BASE}/${id}/cancel`, data)
}
